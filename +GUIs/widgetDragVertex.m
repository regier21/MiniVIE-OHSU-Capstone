classdef widgetDragVertex < hgsetget
    % create a widgetDragVertex tool that will callback on motion events
    % Usage: mySliderHandle = widgetDragVertex(myAxis)
    ...
    % mySliderHandle = GUIs.widgetDragVertex(gca);
    ...
    properties (Access = public)
        ...
        PersistValue = 0;  % Value that the widgetDragVertex will jump to on button up
        ButtonDownFcn = '';
        ButtonMotionFcn = '';
        ButtonUpFcn = '';
    ...
    methods
        function obj = widgetDragVertex(varargin)
            if nargin > 0
                for i=1:2:nargin-1
        ...
        function set.Range(obj,value)
            ...
            if obj.Range(1) > obj.Range(2)
                warning('widgetDragVertex:invalidRange','Invalid Range');
            end
...
function set_line_position(obj)
    ...
    set(obj.hLine,'XData',val,'YData',[0.5]);
